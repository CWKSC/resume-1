...
jetbrains = paragraph
  ...
  , itemize
    ...
    , simple $ "\\item Created a debugger for inspecting bidirectional type-checking and an extensible REPL framework with both CLI and IntelliJ IDEA implementations."
    ]
  ]
...
arendVSCode = paragraph
  ...
  , cn "Arend 语言的 VSCode 扩展，基于实验性的 Arend 语言服务实现。"
  , en "VSCode extension for the Arend language, build on top of the experimental implementation of the Arend language server."
  ]
...
jimgui = paragraph
  ...
  , cn $ "JNI 实现的 \\textit{dear imgui} 接口，通过手写工具生成 FFI 接口，利用 HotSpot 虚拟机的 Critical Native 特性优化 JNI 性能，"
...
typeTheory = paragraph
  ...
  , cn "熟悉 Idris, Agda (\\textbf{3 年}经验，开发组成员)，F$\\star$, Arend （前开发组成员）和一些 Coq。"
  , en "familiar with Idris, Agda (\\textbf{3 years} of experience, contributor),"
  , en "F$\\star$, Arend (prior member) and some Coq."
...
resume = paragraph
  ...
  , section "其他" "Miscellaneous"
    ...
    , en $ "\\item Get the latest one-page version of this resume: " ++ enSimple
    , en $ "\\item Get the latest complete version of this resume: " ++ enElab
